#!/usr/bin/env python3
"""
BASELINE AUTONOMOUS EXPLORATION SYSTEM (No CBF)
...
class AutonomousExplorer(Node):
    def __init__(self):
        ...
        self.declare_parameters("", [
            ("min_frontier_size", 1),
            ("frontier_search_rate", 1.0),
            ("exploration_complete_threshold", 0.95),
            ...
            ("frontier_weight_size", 1.0),
            ("frontier_weight_distance", 0.2),
            ("ig_sensor_range", 3.0),
        ...
        self.create_timer(120.0, self._mission_timeout_cb)

        self.get_logger().info("BASELINE EXPLORER (RRT*-APF, NO CBF) ONLINE!")
    ...
    def frontier_search_loop(self):
        if self.map_data is None or not self.altitude_ready:
            return
    ...
    def find_frontiers(self, search_radius=None):
        ...
        free_cells = (map_slice == 0)
        unknown_cells = (map_slice == -1)
    ...
    def select_best_frontier(self, frontiers):
        ...
        for cluster in frontiers:
            ...
            approach_score = self.score_approach_corridor(
                self.current_x, self.current_y, centroid_x, centroid_y)
            score = (self.w_size * size_score +
                     self.w_dist * distance_score * 100 +
                     self.w_approach * approach_score * 100)
    ...
    def rrt_star(self, start, goal, stop_event=None):
        ...
        for i in range(self.rrt_max_iter):
            ...
            if math.hypot(new_x - goal[0], new_y - goal[1]) < self.rrt_step_size:
                candidate = RRTNode(goal[0], goal[1])
                candidate.parent = new_node
                candidate.cost = new_node.cost + math.hypot(goal[0] - new_x, goal[1] - new_y)
                if best_goal_node is None or candidate.cost < best_goal_node.cost:
                    best_goal_node = candidate
                    best_found_iter = i

            if best_goal_node is not None and (i - best_found_iter) > improve_iters:
                break
    ...
    def potential_field_control(self):
        if not self.waypoints or self.inflated_map is None:
            return
        if self.mission_timed_out or not self.altitude_ready:
            self.publish_zero_velocity()
            return
        ...
        self._last_apf_forces = {
            "att": f_att, "rep_obs": f_rep,
            "rep_drone": f_rep_drone, "total": (fx, fy),
        }

        force_mag = math.hypot(fx, fy)

        # Local minimum detection + escape
    ...
    def _publish_sdf_metrics(self):
        ...
        self.metrics_cbf_infeasible_pub.publish(zero_i)
        self.metrics_cbf_delta_pub.publish(zero_f)
        self.metrics_cbf_speed_clip_pub.publish(false_b)
        self.metrics_cbf_solve_time_pub.publish(zero_f)
    ...
    def attractive_force(self, target_x, target_y):
        ...
        return (self.k_att * d_switch * d_hat_x, self.k_att * d_switch * d_hat_y)
    ...
    def repulsive_force(self):
        ...
        if gx is None:
            return (0.0, 0.0)
        d = max(float(self.distance_field[gy, gx]), d_min)
        if d >= d0:
            return (0.0, 0.0)
        ...
        def _d(r, c):
            return float(self.distance_field[max(0, min(H-1, r)), max(0, min(W-1, c))])
        ...
        if grad_mag < 1e-6:
            return (0.0, 0.0)
        return (scalar * grad_x / grad_mag, scalar * grad_y / grad_mag)
    ...
    def publish_path_viz(self, path):
        ...
        waypoints_msg.header = msg.header
        for (x, y) in path:
            p = PoseStamped()
            p.pose.position.x = x
            p.pose.position.y = y
    ...
    def publish_frontier_markers(self, frontiers, best_frontier):
        ...
        for i, cluster in enumerate(frontiers):
            ...
            marker.color = (ColorRGBA(r=0.0, g=1.0, b=0.0, a=1.0) if is_best
                            else ColorRGBA(r=1.0, g=1.0, b=0.0, a=0.5))
    ...
    def _publish_apf_markers(self):
        ...
        forces = {
            0: ("att",       self._last_apf_forces["att"],       (0.0, 1.0, 0.0)),
            1: ("rep_obs",   self._last_apf_forces["rep_obs"],   (1.0, 0.0, 0.0)),
            2: ("rep_drone", self._last_apf_forces["rep_drone"], (1.0, 0.5, 0.0)),
            3: ("total",     self._last_apf_forces["total"],     (0.0, 0.0, 1.0)),
        }
        scale = 2.0
        for id_, (label, (fx, fy), color) in forces.items():
            ...
            m.type = Marker.ARROW
            m.action = Marker.ADD
            start = Point(x=self.current_x, y=self.current_y, z=0.5)
            end = Point(
                x=self.current_x + (fx / mag) * scale * math.tanh(mag),
                y=self.current_y + (fy / mag) * scale * math.tanh(mag),
                z=0.5)
            m.points = [start, end]
